...
long add_parallel(const char *numbers) {
    long sum = 0;
    //gets the size of the threads
    long chunk_size = Num_To_Add / omp_get_max_threads();
#pragma omp parallel num_threads(omp_get_max_threads())
    {
        //holds the sum of the thread
        long my_sum = 0;
        int n = omp_get_thread_num();
        //gets the first index of the thread
        long my_first_i = n * chunk_size;
        //gets the last index of the thread
        long my_last_i = my_first_i + chunk_size;
#pragma omp parallel for
        for (long my_i = my_first_i; my_i < my_last_i; my_i++) {
            my_sum += numbers[my_i];
        }
#pragma omp critical
            //gets the sums of the threads and adds them together
            sum += my_sum;
        }
    return sum;
}
